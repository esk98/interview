...
- [Типы данных в JavaScript?](https://youtu.be/ycYp7CYOnO0?t=471)
  <br>
  В JS существует 8 основных типов<br>
  Это `string`. <br>
  `number`, причём в этот тип входит в целочисленные значения так и значение с плавающей запятой.<br>
  `Bigint` была введена для того чтобы дать разработчикам возможность работать с числами произвольной длины индикатор `Bigint` это N на конце числа. <br>
  `boolean` значение, true либо false.<br>
  `symbol` это уникальный идентификатор объекта<br>
  `Null` и `underfight` показывают отсутствующие данные и хоть `null` это отдельный тип, но если проверить это через оператор `type of` то получим объект. <br>
  Это ошибка которая сложилась исторически.<br>
  Определить тип данных можно с помощью упомянутого оператора `type of`
